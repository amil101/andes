...
//        if (messageContext.getMessage().hasArray()) {
            ...
            ByteBuffer messageData = messageContext.getMessage();
            ...
            AndesMessageMetadata messageHeader = MQTTUtils
                    .convertToAndesHeader(messageID, messageContext.getTopic(), messageContext.getQosLevel().getValue(),
                            messageData.capacity(), messageContext.isRetain(), publisher);
            ...
            andesMessage.addMessagePartList(messageData);
            Andes.getInstance()
                    .messageReceived(andesMessage, publisher.getChannel(), messageContext.getPubAckHandler());
...
//        }
        ...
        try {
            ...
            if (mqttTopicSubscriber.isDurable()) {
                ...
                InboundQueueEvent createQueueEvent = new InboundQueueEvent(
                        MQTTUtils.getTopicSpecificQueueName(clientID, topic), username, false, true, ProtocolType.MQTT,
    ...
    public void sendRetainedMessagesToSubscriber(String topic, String subscriptionID, QOSLevel qos,
            UUID subscriptionChannelID) throws MQTTException {
        ...
        try {
            ...
            for (DeliverableAndesMetadata metadata : metadataList) {
                ...
                MQTTopicManager.getInstance()
                        .distributeMessageToSubscriber(topic, message, metadata.getMessageID(), metadata.getQosLevel(),
                                metadata.isRetain(), subscriptionID, qos.getValue(), metadata);
    ...
    public void removeSubscriber(MQTTopicManager channel, String subscribedTopic, String username,
            String subscriptionChannelID, UUID subscriberChannel, boolean isCleanSession, String mqttClientID,
            QOSLevel qosLevel) throws MQTTException {
        try {
    ...
    private MQTTLocalSubscription createSubscription(String wildcardDestination, MQTTopicManager channel,
            String mqttClientID, int qos, UUID subscriptionChannelID, boolean isActive, boolean cleanSession)
            throws MQTTException {

        boolean durable = MQTTUtils.isDurable(cleanSession, qos);

        MQTTLocalSubscription outBoundTopicSubscription = new MQTTLocalSubscription(wildcardDestination,
                subscriptionChannelID, isActive, durable);
    ...
    private LocalSubscription createLocalSubscription(MQTTLocalSubscription mqttLocalSubscription, String topic,
        ...
        LocalSubscription localSubscription = AndesUtils
                .createLocalSubscription(mqttLocalSubscription, queueIdentifier, topic, true, isDurable, subscribedNode,
                        subscribedTime, targetQueue, targetQueueOwner, targetQueueBoundExchange,
                        targetQueueBoundExchangeType, isTargetQueueBoundAutoDeletable, hasExternalSubscriptions,
                        destinationType);
